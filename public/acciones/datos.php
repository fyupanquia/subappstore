...
define("DB_DATABASE", "u359017581_sasp");
	...
	function estadisticas(){
		$mysqli = new mysqli(DB_HOST,DB_USER,DB_PASSWORD,DB_DATABASE) or die("Error " . mysqli_error($link)); 
	...
	function conectados(){
		...
		$response ='';
		$consulta = $mysqli->query("select id from conectados");
	...
	function torta(){
		...
		$consulta = $mysqli->query("select id from productos where precio");
		$todos  = $consulta->num_rows;

		$consulta = $mysqli->query("select id from productos where precio > 0 and precio < 501");
		$_500  = $consulta->num_rows;

		$consulta = $mysqli->query("select id from productos where precio > 500 and precio < 1001");
		$_1000  = $consulta->num_rows;

		$consulta = $mysqli->query("select id from productos where precio > 1000 and precio < 1501");
		$_1500  = $consulta->num_rows;

		$consulta = $mysqli->query("select id from productos where precio > 1500 and precio < 2001");
		$_2000  = $consulta->num_rows;

		$consulta = $mysqli->query("select id from productos where precio > 2000 and precio < 2501");
		$_2500  = $consulta->num_rows;

		$consulta = $mysqli->query("select id from productos where precio > 2500");
