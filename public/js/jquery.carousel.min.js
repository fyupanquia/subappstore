...
*/
/* pack : 1 */
;(function($){$.fn.kirousel=function(params){var params=$.extend({direction:"horizontal",loop:false,dispItems:1,pagination:false,paginationPosition:"inside",nextBtn:"<span>Next</span>",prevBtn:"<span>Previous</span>",btnsPosition:"inside",nextBtnInsert:"appendTo",prevBtnInsert:"prependTo",autoSlide:false,autoSlideInterval:3000,delayAutoSlide:3000,combinedClasses:false,effect:"slide",slideEasing:"swing",animSpeed:"normal",equalWidths:"true"},params);if(params.btnsPosition=="outside"){params.prevBtnInsert="insertBefore";params.nextBtnInsert="insertAfter";}
return this.each(function(){var env={$elts:{},params:params,launchOnLoad:[]};env.$elts.kirousel=$(this).addClass("js");env.$elts.content=$(this).find(">*").css({position:"absolute","top":0});env.$elts.wrap=env.$elts.content.wrap('<div class="kirousel-wrap"></div>').parent().css({overflow:"hidden",position:"relative"});env.steps={first:0,count:env.$elts.content.find(">*").length};env.steps.last=env.steps.count-1;env.$elts.prevBtn=$(params.prevBtn)[params.prevBtnInsert](env.$elts.kirousel).addClass("kirousel-control previous kirousel-previous").data("firstStep",-(env.params.dispItems));env.$elts.nextBtn=$(params.nextBtn)[params.nextBtnInsert](env.$elts.kirousel).addClass("kirousel-control next kirousel-next").data("firstStep",env.params.dispItems);initButtonsEvents(env,function(e){slide(e,this,env);});if(env.params.pagination)initPagination(env);$(function(){var $firstItem=env.$elts.content.find(">*:eq(0)");env.itemWidth=$firstItem.outerWidth();if(params.direction=="vertical"){env.contentWidth=env.itemWidth;}else{if(params.equalWidths){env.contentWidth=env.itemWidth*env.steps.count;}else{env.contentWidth=(function(){var totalWidth=0;env.$elts.content.find(">*").each(function(){totalWidth+=$(this).outerWidth();});return totalWidth;})();}}
...
if(env.params.pagination){env.$elts.paginationBtns.removeClass("active").filter(function(){return($(this).data("firstStep")==env.steps.first)}).addClass("active");}};function initButtonsEvents(env,slideEvent){env.$elts.nextBtn.add(env.$elts.prevBtn).bind("enable",function(){var $this=$(this).bind("click",slideEvent).removeClass("disabled");if(env.params.combinedClasses){$this.removeClass("next-disabled previous-disabled");}}).bind("disable",function(){var $this=$(this).unbind("click").addClass("disabled");if(env.params.combinedClasses){if($this.is(".next")){$this.addClass("next-disabled");}else if($this.is(".previous")){$this.addClass("previous-disabled");}}});env.$elts.nextBtn.add(env.$elts.prevBtn).hover(function(){$(this).addClass("hover");},function(){$(this).removeClass("hover");});};
	...
	function initPagination(env){
	  	env.$elts.pagination = $('<div class="center-wrap"><div class="kirousel-pagination"><p></p></div></div>')[((env.params.paginationPosition=="outside")?"insertAfter":"appendTo")](env.$elts.kirousel).find("p");
	  			
		env.$elts.paginationBtns = $([]);
		
		env.$elts.content.find("li").each(function(i){
			if (i % env.params.dispItems == 0) {
				if (i +1  < 10) env.$elts.paginationBtns = env.$elts.paginationBtns.add( $('<a role="button"><span>0'+(i+1)+'</span></a>').data("firstStep", i) );
				else env.$elts.paginationBtns = env.$elts.paginationBtns.add( $('<a role="button"><span>'+( i + 1 )+'</span></a>').data("firstStep", i) );
				env.$elts.paginationBtns = env.$elts.paginationBtns.add( $('<span style="white-space: normal!important;">&shy; \n</span>').data("firstStep", i) );

				/*
				if (env.$elts.paginationBtns.length +1  < 10) env.$elts.paginationBtns = env.$elts.paginationBtns.add( $('<a role="button"><span>0'+( env.$elts.paginationBtns.length + 1 )+' - '+(i+1)+'</span></a>').data("firstStep", i) );
				else env.$elts.paginationBtns = env.$elts.paginationBtns.add( $('<a role="button"><span>'+( env.$elts.paginationBtns.length + 1 )+'</span></a>').data("firstStep", i) );
				env.$elts.paginationBtns = env.$elts.paginationBtns.add( $('<span style="white-space: normal!important;"> </span>').data("firstStep", i) );
				*/
