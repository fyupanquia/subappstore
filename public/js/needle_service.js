...
N.scheme="http";N.version="0FEB3E";N.service='alienware.needle.com';N.cws="width=485,height=525";N.cm="chatreq";N.cfid="reco";N.ls=0;N.ss=0;N.pl=1;N.ui=0;N.bt=0;N.it=0;N.ots="";N.pct="";N.lct="L";N.tsct=0;N.mps=300000;N.st=new Date().getTime();N.tNow=new Date().getTime();N.tAct=0;N.invites=N.invites||{};N.PATCH_INJECT_RECO_TYPE="EMPTY";var storefail,uid;try{uid=new Date;(N.ls=window.localStorage).setItem(uid,uid);(N.ss=window.sessionStorage).setItem(uid,uid);storefail=(N.ls.getItem(uid)!=uid||N.ss.getItem(uid)!=uid);N.ls.removeItem(uid);N.ss.removeItem(uid);if(storefail){N.ls=false;N.ss=false;}}catch(e){}
...
N.nps='2';else
N.nps='3';}
N.linkVisit=function(cr,ur,fr,as_hash){var i,cls,umatch,smatch,hmatch,fmatch;var d=document;var href,newhref;var links=d.getElementsByTagName('A');var vlinks=[],llinks;var qvid='ndlvid='+N.vid;for(i=0;i<links.length;i++){cls=links[i].className;if(!links[i].getAttribute('href')){continue;}
if(cr instanceof RegExp){if(cr.test(cls)){vlinks.push(links[i]);}}else{vlinks.push(links[i]);}}
if(vlinks.length&&ur instanceof RegExp){llinks=[];for(i=0;i<vlinks.length;i++){if(ur.test(vlinks[i].getAttribute('href'))){llinks.push(vlinks[i]);}}
vlinks=llinks;}
for(i=0;i<vlinks.length;i++){href=vlinks[i].getAttribute('href');umatch=href.match(/^[^\?#]+/);smatch=href.match(/\?[^#]+/);hmatch=href.match(/#[^?]+/);newhref=umatch[0];if(as_hash){if(smatch){newhref+=smatch[0];}
if(hmatch){newhref+=hmatch[0]+'&'+qvid;}else{newhref+='#'+qvid;}}else{if(smatch){newhref+=smatch[0]+'&'+qvid;}else{newhref+='?'+qvid;}
if(hmatch){newhref+=hmatch[0];}}
vlinks[i].href=newhref;}
if(fr instanceof RegExp){for(i=0;i<d.forms.length;i++){href=d.forms[i].action;if(fr.test(href)){umatch=href.match(/^[^\?#]+/);smatch=href.match(/\?[^#]+/);hmatch=href.match(/#[^?]+/);newhref=umatch[0];if(as_hash){if(smatch){newhref+=smatch[0];}
if(hmatch){newhref+=hmatch[0]+'&'+qvid;}else{newhref+='#'+qvid;}}else{if(smatch){newhref+=smatch[0]+'&'+qvid;}else{newhref+='?'+qvid;}
if(hmatch){newhref+=hmatch[0];}}
d.forms[i].action=newhref;}}}}
N.analyticsCall=function(a,v,e){if(typeof s_gi!="function"){N.PageUpdate('A',"&call="+a+"&error=1");return;}
var s=window.s_dell;if(typeof s.tl!="function"){N.PageUpdate('A',"&call="+a+"&error=2");return;}
if(a==='bc'){return;}else if(a==='qc'){s.linkTrackVars='prop49,eVar62';s.eVar62='NeedleChat';s.prop49="?c=us&l=en&s=dhs";}else{N.PageUpdate('A',"&call="+a+"&error=3");return;}
try{s.tl(true,'o','needle');s.linkTrackVars='None';N.PageUpdate('A',"&call="+a);}catch(err){N.PageUpdate('A',"&call="+a+'&error=4');}}
np.lz='alienware';if(!N.loaded&&N.nps<3&&N.getCookie("needleopt")==null){if("onfocusin"in document){N.oldOnFocus=document.onfocusin;N.oldOnBlur=document.onfocusout;document.onfocusin=N.onFocus;document.onfocusout=N.onBlur;}else{N.oldOnFocus=window.onfocus;N.oldOnBlur=window.onblur;window.onfocus=N.onFocus;window.onblur=N.onBlur;}
